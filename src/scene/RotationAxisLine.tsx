...
function paintNorth(ctx: CanvasRenderingContext2D, width: number, height: number, tilt: string) {
...
function paintSouth(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ctx.clearRect(0, 0, width, height)
  ctx.font = '600 36px ui-sans-serif, system-ui, sans-serif'
...
function spriteWorldHeight(camera: PerspectiveCamera, x: number, y: number, z: number, viewHeight: number) {
  const dist = Math.hypot(camera.position.x - x, camera.position.y - y, camera.position.z - z)
  return 2 * Math.tan((camera.fov * Math.PI) / 360) * dist * (LABEL_PIXELS / Math.max(viewHeight, 1))
}
...
export function RotationAxisLine({
  ...
  useFrame(({ camera }) => {
    ...
    const northExtra = spriteWorldHeight(persp, northX, northY, northZ, viewHeight) * LABEL_GAP
    const southExtra = spriteWorldHeight(persp, southX, southY, southZ, viewHeight) * LABEL_GAP
