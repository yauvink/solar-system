...
const SHOW_NAMES_KEY = "solar-system:showNames";
...
function readStoredScale(): ScaleSettings {
  try {
    ...
    return {
      auInUnits: clamp(
        Number(parsed.auInUnits) || DEFAULT_SCALE.auInUnits,
        20,
        250,
      ),
      bodyScale: clamp(
...
export default function App() {
  ...
  const { store, date, live, positions, goLive, setCustomDate, setSpeed } =
    useBodyPositions(scale.auInUnits, scale.bodyScale);
  const radii = useMemo(() => getBodyRadii(scale), [scale]);
  const [focus, setFocus] = useState<FocusId>("earth");
  const [focusFrom, setFocusFrom] = useState<FocusId>("earth");
  const focusRef = useRef<FocusId>("earth");
  ...
  const [showAxes, setShowAxes] = useState(() =>
    readStoredFlag(SHOW_AXES_KEY, true),
  );
  const [showNames, setShowNames] = useState(() =>
    readStoredFlag(SHOW_NAMES_KEY, true),
  );
  ...
  const handleShowNames = useCallback((value: boolean) => {
    setShowNames(value);
  }, []);
  ...
  useEffect(() => {
    writeStoredFlag(SHOW_NAMES_KEY, showNames);
  }, [showNames]);
  ...
  return (
    <div className="app">
      ...
      <Controls
        ...
        showNames={showNames}
        onShowNamesChange={handleShowNames}
        geoStatus={geoStatus}
